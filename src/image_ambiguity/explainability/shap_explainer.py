...
class SHAPExplainer:
    ...
    def force_plot(
        ...
        *,
        ...
        plt = _get_pyplot(show)
        row = self._slice_row(shap_values, row_index, class_index)
        # Round displayed feature values so the matplotlib-rendered force
        # plot's inline labels stay legible (full float64 precision makes
        # them overlap and run together).
        row.data = np.round(np.asarray(row.data), 3)
