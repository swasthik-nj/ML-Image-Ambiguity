...
def flatten_generated_captions(generated: dict[str, list[str]]) -> list[str]:
    ...
    # Include any unexpected strategy keys last.
    for strategy, values in generated.items():
...
def load_caption_cache(path: str | Path) -> dict[int, list[str]]:
    """Load ``{image_id: [captions...]}`` from JSON."""
...
def save_caption_cache(
    ...
    """Persist caption cache as a JSON object with string keys."""
    payload = {str(image_id): captions for image_id, captions in sorted(
...
def collect_blip_captions(
    ...
    *,
    ...
    """Generate BLIP captions for ``image_ids``, reusing cache when possible.

    Args:
        loader: Loaded :class:`CocoDatasetLoader` used to open images.
        image_ids: COCO image ids to caption.
        generator: Optional preconfigured BLIP generator.
        cache_path: JSON cache path, or ``None`` to disable caching.
        strategies: Decoding strategies to run for uncached images.
        force: If ``True``, regenerate captions even when cached.
        seed: Random seed for stochastic decoding strategies.

    Returns:
        Mapping of image id → flattened unique BLIP captions.
    """
    ...
    if missing:
        ...
        with timed(f"blip_batch:n={len(missing)}"):
            for index, image_id in enumerate(missing, start=1):
                ...
                except Exception:  # noqa: BLE001 - keep batch resilient
                    logger.exception(
